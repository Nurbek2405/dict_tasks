...
'''prices = {"apple": 50, "banana": 20, "cherry": 30, "date": 40}
...
'''
Задача 15: Словарь с подсчётом символов строки
Условие:
Напишите программу, которая создаёт словарь, подсчитывающий, сколько раз каждый символ встречается в строке.

Входные данные:
Введите строку s = "hello world".

Подсказка:

Пройдитесь по строке с помощью цикла for.
Используйте метод .get() или проверку наличия ключа в словаре для подсчёта.
Ожидаемый вывод:
